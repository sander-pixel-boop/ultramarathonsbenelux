...
## 2025-05-11 - Module-Level Caching for Static Data
**Learning:** In Next.js applications, static data files (like `races.json`) are often read and parsed repeatedly across different lifecycle functions (`getStaticProps`, `getStaticPaths`, `generateMetadata`, etc.) during the build process. This redundancy causes significant I/O and CPU overhead, especially as the dataset grows.
**Action:** Implement module-level caching for shared data sources. Use a simple `let cachedData = null;` variable at the top level of the file and a helper function to populate it on first access. This ensures the file is read and parsed exactly once per process, yielding massive performance gains (e.g., >160x speedup in parsing-intensive benchmarks).
